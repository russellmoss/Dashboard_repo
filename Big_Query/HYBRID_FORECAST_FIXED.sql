-- Step 5.1: Hybrid Forecast Pipeline (FIXED VERSION)
-- Combines ARIMA models for healthy segments with heuristic for sparse segments
-- This version generates forecast for Oct 1 - Dec 31 (not from CURRENT_DATE)
...
WITH 
-- Generate date spine for Q4 2025 (Oct 1 - Dec 31)
date_spine AS (
  SELECT date_day
  FROM UNNEST(
    GENERATE_DATE_ARRAY(
...
-- Calculate 30-day rolling average for sparse segments
sparse_averages AS (
  WITH healthy_segments AS (
  ...
  WHERE h.Channel_Grouping_Name IS NULL
),
...
heuristic_forecast AS (
  SELECT
    ...
    sa.Original_source,
    sa.mqls_30day_avg AS mqls_forecast_raw,
    sa.mqls_30day_avg * 0.7 AS mqls_lower_raw,
    sa.mqls_30day_avg * 1.3 AS mqls_upper_raw
  FROM date_spine d
  CROSS JOIN sparse_averages sa
),
...
heuristic_sql_forecast AS (
  SELECT
    ...
    sa.Channel_Grouping_Name,
    sa.Original_source,
    sa.sqls_30day_avg AS sqls_forecast_raw,
    sa.sqls_30day_avg * 0.7 AS sqls_lower_raw,
    sa.sqls_30day_avg * 1.3 AS sqls_upper_raw
  FROM date_spine d
  CROSS JOIN sparse_averages sa
),
