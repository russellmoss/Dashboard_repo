-- vw_sgm_open_sqos_detail: Detailed view of all open SQOs by SGM
-- This view provides detailed information about each open SQO for verification purposes
-- Shows: SGM name, Opportunity details, Stage, AUM metrics, and days open since becoming SQO
-- UPDATED: V2 Logic - Dynamic Valuation, Deal-Size Dependent Stale Logic, Deal-Size + Stage Dependent Velocity
  ...
  SELECT
    ...
    CASE
      WHEN o.Date_Became_SQO__c IS NOT NULL AND o.advisor_join_date__c IS NULL THEN
        CASE
          WHEN (CASE WHEN o.Margin_AUM__c IS NOT NULL AND o.Margin_AUM__c > 0 THEN o.Margin_AUM__c ELSE COALESCE(o.Underwritten_AUM__c / vp.dyn_und_div, o.Amount / vp.dyn_amt_div) END) >= 30000000 THEN
            CASE
            ...
            END
          WHEN (CASE WHEN o.Margin_AUM__c IS NOT NULL AND o.Margin_AUM__c > 0 THEN o.Margin_AUM__c ELSE COALESCE(o.Underwritten_AUM__c / vp.dyn_und_div, o.Amount / vp.dyn_amt_div) END) >= 15000000 THEN
            CASE
              WHEN o.Stage_Entered_Signed__c IS NOT NULL AND DATE(o.Stage_Entered_Signed__c) <= c.current_date
...
SELECT
  ...
  -- Forecast quarter assignment (matching vw_sgm_capacity_coverage_with_forecast logic exactly)
  -- Uses inline date calculation to match forecast view logic
  CASE
    WHEN ob.Date_Became_SQO__c IS NOT NULL AND ob.advisor_join_date__c IS NULL THEN
      CASE
        -- Determine deal size bucket for cycle time calculation (matching forecast view - no multiplication)
        WHEN (CASE WHEN ob.Margin_AUM__c IS NOT NULL AND ob.Margin_AUM__c > 0 THEN ob.Margin_AUM__c ELSE COALESCE(ob.Underwritten_AUM__c / vp.dyn_und_div, ob.Amount / vp.dyn_amt_div) END) >= 30000000 THEN
          -- Enterprise (>$30M)
          CASE
            WHEN ob.Stage_Entered_Signed__c IS NOT NULL AND DATE(ob.Stage_Entered_Signed__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Signed__c), INTERVAL 38 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Signed__c), INTERVAL 38 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            WHEN ob.Stage_Entered_Negotiating__c IS NOT NULL AND DATE(ob.Stage_Entered_Negotiating__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Negotiating__c), INTERVAL 49 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Negotiating__c), INTERVAL 49 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            WHEN ob.Stage_Entered_Sales_Process__c IS NOT NULL AND DATE(ob.Stage_Entered_Sales_Process__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Sales_Process__c), INTERVAL 94 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Sales_Process__c), INTERVAL 94 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            ELSE
              CASE
                WHEN DATE_ADD(DATE(ob.Date_Became_SQO__c), INTERVAL 120 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Date_Became_SQO__c), INTERVAL 120 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
          END
        WHEN (CASE WHEN ob.Margin_AUM__c IS NOT NULL AND ob.Margin_AUM__c > 0 THEN ob.Margin_AUM__c ELSE COALESCE(ob.Underwritten_AUM__c / vp.dyn_und_div, ob.Amount / vp.dyn_amt_div) END) >= 15000000 THEN
          -- Large ($15M-$30M)
          CASE
            WHEN ob.Stage_Entered_Signed__c IS NOT NULL AND DATE(ob.Stage_Entered_Signed__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Signed__c), INTERVAL 18 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Signed__c), INTERVAL 18 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            WHEN ob.Stage_Entered_Negotiating__c IS NOT NULL AND DATE(ob.Stage_Entered_Negotiating__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Negotiating__c), INTERVAL 37 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Negotiating__c), INTERVAL 37 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            WHEN ob.Stage_Entered_Sales_Process__c IS NOT NULL AND DATE(ob.Stage_Entered_Sales_Process__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Sales_Process__c), INTERVAL 66 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Sales_Process__c), INTERVAL 66 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            ELSE
              CASE
                WHEN DATE_ADD(DATE(ob.Date_Became_SQO__c), INTERVAL 90 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Date_Became_SQO__c), INTERVAL 90 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
          END
        ELSE
          ...
          CASE
            WHEN ob.Stage_Entered_Signed__c IS NOT NULL AND DATE(ob.Stage_Entered_Signed__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Signed__c), INTERVAL 10 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Signed__c), INTERVAL 10 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            WHEN ob.Stage_Entered_Negotiating__c IS NOT NULL AND DATE(ob.Stage_Entered_Negotiating__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Negotiating__c), INTERVAL 18 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Negotiating__c), INTERVAL 18 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            WHEN ob.Stage_Entered_Sales_Process__c IS NOT NULL AND DATE(ob.Stage_Entered_Sales_Process__c) <= c.current_date THEN
              CASE
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Sales_Process__c), INTERVAL 49 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Stage_Entered_Sales_Process__c), INTERVAL 49 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
            ELSE
              CASE
                WHEN DATE_ADD(DATE(ob.Date_Became_SQO__c), INTERVAL 50 DAY) <= c.current_quarter_end THEN 'Current Quarter'
                WHEN DATE_ADD(DATE(ob.Date_Became_SQO__c), INTERVAL 50 DAY) <= c.next_quarter_end THEN 'Next Quarter'
                ELSE 'Beyond Next Quarter'
              END
          END
      END
  ...
  -- Formula: estimated_margin_aum × stage_probability × stale_decay_factor
  ...
  CASE
    WHEN ob.IsClosed = FALSE 
      ...
      -- Check if projected date is in Current or Next Quarter (using velocity_projected_close_date)
      AND (
