-- vw_sgm_capacity_coverage_with_forecast: Enhanced capacity coverage view with quarterly forecasts
...
SGM_Quarterly_Forecast AS (
  SELECT
    o.sgm_name,
    -- Current Quarter Forecast: Expected Margin AUM × Stage Probability × Stale Decay
    ...
    SUM(CASE 
      ...
      THEN o.estimated_margin_aum * o.stage_probability * 
        CASE 
          ...
          -- This primarily affects Enterprise deals (≥$30M) which have 240-day stale threshold
    ...
    -- Next Quarter Forecast: Expected Margin AUM × Stage Probability × Stale Decay
    SUM(CASE 
      WHEN o.is_in_pipeline = 1
