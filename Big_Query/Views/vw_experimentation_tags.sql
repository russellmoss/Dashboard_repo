CREATE OR REPLACE VIEW `savvy-gtm-analytics.savvy_analytics.vw_experimentation_tags` AS
WITH Lead_Base AS (
  SELECT
    Full_prospect_id__c,
...
Opp_Base AS (
  SELECT
    ...
    o.OwnerId,
    o.Experimentation_Tag__c AS Opportunity_Experimentation_Tag__c
...
, Combined AS (
  SELECT
    ...
    l.Company,
    l.lead_list_name__c,
    l.Lead_Experimentation_Tag__c,
    o.Opportunity_Experimentation_Tag__c,
    ...
    ARRAY(
      ...
      FROM UNNEST(SPLIT(COALESCE(o.Opportunity_Experimentation_Tag__c, l.Lead_Experimentation_Tag__c, ''), ';')) AS tag
      WHERE TRIM(tag) != ''
    ...
    CASE WHEN o.Stage_Entered_Signed__c IS NOT NULL THEN 1 ELSE 0 END AS is_signed,
    ...
    DATE(COALESCE(
      ...
      o.Date_Became_SQO__c,
      TIMESTAMP(o.advisor_join_date__c)
    )) AS prospect_stage_date,
    DATE(COALESCE(l.stage_entered_contacting__c, o.Opp_CreatedDate)) AS contacting_stage_date,
    DATE(l.mql_stage_entered_ts) AS mql_stage_date,
    DATE(l.converted_date_raw) AS sql_stage_date,
    DATE(o.Date_Became_SQO__c) AS sqo_stage_date,
    DATE(o.Stage_Entered_Signed__c) AS signed_stage_date,
    DATE(o.advisor_join_date__c) AS joined_stage_date
  FROM Lead_Base l
...
FROM Combined c
LEFT JOIN UNNEST(
  IF(ARRAY_LENGTH(c.Experimentation_Tag_List) = 0, [CAST(NULL AS STRING)], c.Experimentation_Tag_List)
) AS tag;
