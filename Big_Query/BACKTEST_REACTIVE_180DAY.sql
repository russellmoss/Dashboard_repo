...
FOR rec IN (
  ...
  EXECUTE IMMEDIATE '''
    ...
    WHERE date_day BETWEEN DATE_SUB(''' || "'" || CAST(rec.train_end_date AS STRING) || "'" || ''', INTERVAL 180 DAY) AND ''' || "'" || CAST(rec.train_end_date AS STRING) || "'" || '''
  ...
  EXECUTE IMMEDIATE '''
    ...
    WHERE label IS NOT NULL AND sql_date BETWEEN DATE_SUB(''' || "'" || CAST(rec.train_end_date AS STRING) || "'" || ''', INTERVAL 180 DAY) AND ''' || "'" || CAST(rec.train_end_date AS STRING) || "'" || '''''';
